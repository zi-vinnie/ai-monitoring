...
A FastAPI service on the monitored machine exposing one API-key-protected endpoint, `GET /screenshot`, which captures **only the monitor currently holding the focused window** and returns it as base64 PNG. The [server](../server) component polls it every 10 minutes.
